...
OUTPUT_FILE="instances_with_disks_and_memory_all_projects.csv"

# Cabeçalho do CSV
echo "Projeto,Nome da Instância,IP Interno,IP Externo,Discos,Memória RAM (GB)" > $OUTPUT_FILE
...
for PROJECT in $PROJECTS; do
  ...
  while IFS=',' read -r INSTANCE_NAME ZONE INTERNAL_IP EXTERNAL_IP; do
    ...
    # Linha formatada
    LINE="$PROJECT,$INSTANCE_NAME,$INTERNAL_IP,$EXTERNAL_IP,\"$DISK_DETAILS\",$MEMORY_RAM_GB"
